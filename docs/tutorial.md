...
In this step we will install Pdm if you are not using it, since the whole project is managed by it.

```bash
pip install pdm
```

In addition, Pdm provides a [custom installer](https://python-pdm.org/docs/#installation) that will install
pdm isolated from the rest of your system by vendorizing its dependencies.
This is the recommended way of installing pdm.
...
Install the new project's local development requirements with `pdm install`:

``` bash
pdm install -E doc -E dev -E test
pdm run tox
```

Pdm will create its own virtualenv isolated from your system and install the dependencies in it.
...
We also launch a smoke test here by running `pdm run tox`. This will run `tox` within created virtual environment,
give you a test report and lint report. You should see no errors except some lint warnings.

You can also activate the virtual environment manually with `pdm shell`, this will create a new shell.
