...
def get_local_path(result):
    return py.path.local(result._project_dir)
...
def test_year_compute_in_license_file(cookies):
    with bake_in_temp_dir(cookies) as result:
        project_path = get_local_path(result)

        license_file_path = project_path.join('LICENSE')
...
def test_bake_with_defaults(cookies):
    with bake_in_temp_dir(cookies) as result:
        project_path = get_local_path(result)
        assert project_path.isdir()
        assert result.exit_code == 0
        assert result.exception is None

        found_toplevel_files = [f.basename for f in project_path.listdir()]
...
def test_bake_selecting_license(cookies, license_info):
    ...
    with bake_in_temp_dir(cookies, extra_context={'open_source_license': license}) as result:
        project_path = get_local_path(result)
        assert target_string in project_path.join('LICENSE').read()
        assert license in project_path.join(_DEPENDENCY_FILE).read()
...
def test_bake_not_open_source(cookies):
    with bake_in_temp_dir(cookies, extra_context={'open_source_license': 'Not open source'}) as result:
        ...
        assert 'License' not in project_path.join('README.md').read()
        assert 'license' not in project_path.join(_DEPENDENCY_FILE).read()
...
def test_docstrings_style(cookies):
    with bake_in_temp_dir(cookies, extra_context={'docstrings_style': 'google'}) as result:
        ...
        flake8_conf_file_apth = project_path.join(".flake8")
...
def test_bake_with_no_console_script(cookies, args):
    ...
    with open(pyproject_path, 'r') as pyproject_file:
        assert ('[project.scripts]' in pyproject_file.read()) == is_present
